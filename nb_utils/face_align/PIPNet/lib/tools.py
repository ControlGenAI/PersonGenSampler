from nb_utils.face_align.PIPNet.FaceBoxesV2.faceboxes_detector import *
...
from nb_utils.face_align.PIPNet.lib.networks import *
from nb_utils.face_align.PIPNet.lib.functions import *
from nb_utils.face_align.PIPNet.reverse_index import ri1, ri2
...
def get_lmk_model(weights_path=None):
    cfg = Config()

    resnet101 = models.resnet101(
        weights=ResNet101_Weights.IMAGENET1K_V1 if cfg.pretrained else None
    )
    net = Pip_resnet101(
        resnet101,
        cfg.num_nb,
    ...
    weights_path = weights_path or os.path.join(os.path.dirname(__file__), './../weights')
    os.makedirs(weights_path, exist_ok=True)

    weight_file = os.path.join(weights_path, 'epoch59.pth')
    if not os.path.exists(weight_file):
        print('Downloading PIPNet weights...')
        torch.hub.download_url_to_file('https://nxt.2a2i.org/index.php/s/Co89yyY7sjNLqsz/download', weight_file)
    state_dict = torch.load(weight_file, map_location=device, weights_only=True)
    net.load_state_dict(state_dict)

    weight_file = os.path.join(weights_path, 'FaceBoxesV2.pth')
    if not os.path.exists(weight_file):
        print('Downloading FaceBoxesV2 weights...')
        torch.hub.download_url_to_file('https://nxt.2a2i.org/index.php/s/pqYP5DCeSoNpJMZ/download', weight_file)
