...
from baselines.elite.datasets import OpenImagesDatasetWithMask
from baselines.elite.train_global import inj_forward_text, th2image, Mapper
...
def inj_forward_crossattention(self: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None, use_local=True):
    assert temb is None, f"temb: {temb} is not supported"
    ...
    if use_local and context is not None and "LOCAL" in context:
        ...
        query_local = self.head_to_batch_dim(query_local)
        key_local = self.head_to_batch_dim(key_local)
        value_local = self.head_to_batch_dim(value_local)
...
def validation(
        example,
        tokenizer, image_encoder, text_encoder, unet, mapper, mapper_local, vae,
        device,
        guidance_scale, guidance_scale_ref=0.0, change_step=-1,
        token_index=0, seed=None, llambda=1, num_steps=100, latents=None
):
    ...
    if seed is None and latents is None:
        latents = torch.randn(
            (example["pixel_values"].shape[0], unet.in_channels, 64, 64)
        )
    elif seed is not None and latents is None:
        generator = torch.manual_seed(seed)
        latents = torch.randn(
            (example["pixel_values"].shape[0], unet.in_channels, 64, 64), generator=generator,
        )
    elif seed is not None and latents is not None:
        raise ValueError('What should I use latents or seed?')
    ...
    if 'input_ids_superclass' in example:
        superclass_embeddings = text_encoder({'input_ids': example['input_ids_superclass']})[0]
    else:
        superclass_embeddings = None
    ...
    for i, t in enumerate(tqdm(scheduler.timesteps)):
        ...
        noise_concept = unet(
            ...
            } | (dict() if mapper_local is None else {
        ...
        if superclass_embeddings is not None:
            noise_superclass = unet(
                latent_model_input,
                t,
                encoder_hidden_states={
                    "CONTEXT_TENSOR": superclass_embeddings,
                }
            ).sample
            value_local_list.clear()

            s_1 = check_guidance_scale(guidance_scale, i)
            s_2 = check_guidance_scale(guidance_scale_ref, i)
            if i >= change_step:
            ...
            noise_pred = noise_pred_uncond + guidance_scale * (
                    noise_concept - noise_pred_uncond
            )
