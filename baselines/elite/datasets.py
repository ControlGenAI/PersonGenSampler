...
class CustomDatasetWithBG(Dataset):
    def __init__(
        ...
        self.image_paths += [
            ...
            if is_image(file_path) and 'bg' not in file_path
        ]
    ...
    def __getitem__(self, i):
        ...
        if self.class_name is not None:
            text_superclass = self.template.format(self.class_name)
            example["text_superclass"] = text_superclass
            example['input_ids_superclass'] = self.tokenizer(
                text_superclass,
                padding="max_length",
                truncation=True,
                max_length=self.tokenizer.model_max_length,
        ...
        mask_path = self.image_paths[i % self.num_images].replace('.jpeg', '.png').replace('.jpg', '.png').replace(
            '.JPEG', '.png')[:-4] + '_bg.png'
        ...
        ref_object_tensor = Image.fromarray(object_tensor.astype('uint8')).resize((224, 224),
        ...
        example["pixel_values_seg"] = torch.nn.functional.interpolate(ref_seg_tensor.unsqueeze(0), size=(128, 128),
...
class OpenImagesDatasetWithMask(OpenImagesDataset):
    ...
    def custom_aug(self, instance_image):
        ...
        return torch.from_numpy(instance_image).permute(2, 0, 1), torch.from_numpy(mask[:, :, None]).permute(2, 0,
                                                                                                             1), add_to_caption
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            ref_object_tensor, ref_seg_tensor = self.aug_cv2(ref_object_tensor.astype('uint8'),
