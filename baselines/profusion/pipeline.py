...
from diffusers.utils.import_utils import is_accelerate_version
...
# Hack to remove classes from docstring
UNet2DConditionModelEmb = None
...
class StableDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            seq_len = uncond_embeds.shape[1]

            uncond_embeds = uncond_embeds.to(dtype=self.text_encoder.dtype, device=device)
    ...
    @torch.no_grad()
    def generate_epsilons(self, time, prompt, ref_prompt,
                          latent_model_input,
                          num_images_per_prompt, device, cross_attention_kwargs):
        # generate epsilons for sampling
        uncond_embeds, prompt_embeds = self._encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            True,
        ...
        if ref_prompt is not None:
            ...
            prompt_embeds = torch.cat([uncond_embeds, prompt_embeds, ref_prompt_embeds])
        else:
    ...
    def __call__(
        ...
        self.check_inputs(
            prompt, height, width, callback_steps, None, None
        )
                        ...
                        for kdx in range(refine_step):
                            ...
                            noise_pred_text_0, noise_pred_uncond, _ = \
                                self.generate_epsilons(time=t,
                                                       prompt=prompt,
                                                       ref_prompt=None,
                                                       latent_model_input=latent_model_input[:latent_model_input.shape[0] // 3 * 2],
...
class StableDiffusionPipelineOFT(StableDiffusionPipeline):
    def __init__(
            self,
            vae: AutoencoderKL,
            tokenizer: CLIPTokenizer,
            ...
            feature_extractor: CLIPFeatureExtractor,
            requires_safety_checker: bool = True,
            original_unet: Optional[UNet2DConditionModel] = None,
            original_text_encoder: Optional[CLIPTextModel] = None
    ):
        super().__init__(
            vae=vae, tokenizer=tokenizer, unet=unet, text_encoder=text_encoder,
    ...
    def _encode_prompt(
        ...
        Args:
            ...
            negative_prompt_embeds (`torch.FloatTensor`, *optional*):
                ...
                argument.
        """
        text_encoder = self.original_text_encoder if use_original_model else self.text_encoder
        ...
        if do_classifier_free_guidance:
            ...
            uncond_embeds = uncond_embeds.to(dtype=text_encoder.dtype, device=device)
    ...
    def generate_epsilons(self, time, prompt, ref_prompt,
        ...
        if ref_prompt is not None:
            uncond_ref_prompt_embeds, ref_prompt_embeds = self._encode_prompt(
                ref_prompt,
                device,
                num_images_per_prompt,
            ...
            prompt_embeds = torch.cat([uncond_embeds, prompt_embeds, uncond_ref_prompt_embeds, ref_prompt_embeds])
        else:
            prompt_embeds = torch.cat([uncond_embeds, prompt_embeds])
        ...
        if ref_prompt is not None:
            noise_pred_uncond, noise_pred_text_0, noise_pred_uncond_ref, noise_pred_ref = noise_pred_0.chunk(4)
        else:
            noise_pred_uncond, noise_pred_text_0 = noise_pred_0.chunk(2)
            noise_pred_uncond_ref = noise_pred_ref = noise_pred_uncond
        return noise_pred_text_0, noise_pred_uncond, noise_pred_uncond_ref, noise_pred_ref
                ...
                if t <= st:
                    ...
                    if ref_prompt is not None or refine_step > 0:
                        ...
                        alpha_prod_t_prev = self.scheduler.alphas_cumprod[
                            prev_t] if prev_t >= 0 else self.scheduler.final_alpha_cumprod
                        variance = self.scheduler._get_variance(t, prev_t)
                        sigma_t = refine_eta * variance ** (0.5)
                        ...
                        for kdx in range(refine_step):
                            if refine_noise is None:
                                ...
                                noise = refine_noise[kdx, i]
                            noise_pred_text_0, noise_pred_uncond, _, _ = \
                                self.generate_epsilons(time=t,
                                                       prompt=prompt,
                                                       ref_prompt=None,
                                                       latent_model_input=latent_model_input[:latent_model_input.shape[0] // 2],
                                                       num_images_per_prompt=num_images_per_prompt, device=device,
                                                       cross_attention_kwargs=cross_attention_kwargs,
                                                       )
                            eps = refine_guidance_scale * (noise_pred_text_0 - noise_pred_uncond) + noise_pred_uncond

                            latents = latents - eps * (sigma_t ** 2 * torch.sqrt(1 - alpha_prod_t)) / (
                                    1 - alpha_prod_t_prev) \
                                      + sigma_t * noise * torch.sqrt(
                    ...
                    noise_pred_text_0, noise_pred_uncond, noise_pred_uncond_ref, noise_pred_ref = \
                        self.generate_epsilons(time=t,
                                               prompt=prompt, ref_prompt=ref_prompt,
                                               latent_model_input=latent_model_input,
                                               num_images_per_prompt=num_images_per_prompt, device=device,
                    ...
                    noise_pred = noise_pred_uncond \
                                 + s_1 * (noise_pred_text_0 - noise_pred_uncond) \
                                 + s_2 * (noise_pred_ref - noise_pred_uncond_ref)
