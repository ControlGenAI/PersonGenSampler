...
from .model.utils import count_trainable_params, params_grad_norm, cast_training_params
from .data.dataset import ImageDataset, DreamBoothDataset, collate_fn, tokenize_prompt, encode_tokens, compute_time_ids
...
class BaseTrainer:
    ...
    def setup_model(self):
        ...
        if self.config.finetune_text_encoder:
            ...
            if self.config.qkv_only:
                ...
                for (name, param) in itertools.chain(
                        ...
                        self.text_encoder_2.named_parameters()
                ):
                    if (
                ...
                te_params_to_optimize = list(self.text_encoder.parameters()) + list(self.text_encoder_2.parameters())
        ...
        te_2_numel = count_trainable_params(self.text_encoder_2, verbose=True)

        print(
            '# params UNet: {0}, # params TE: {1}, # params TE_2: {2}, # params: {3}'.format(
                unet_numel, te_numel, te_2_numel, unet_numel + te_numel + te_2_numel
            )
        )
    ...
    def save_model(self, epoch):
        ...
        if self.config.finetune_text_encoder:
            ...
            torch.save(self.text_encoder_2.state_dict(), os.path.join(save_path, 'text_encoder_2.bin'))
        if self.config.finetune_unet:
...
class SVDDiffTrainer(BaseTrainer):
    ...
    def setup_model(self):
        ...
        if self.config.finetune_text_encoder:
            ...
            self.text_encoder_2.to(self.accelerator.device, dtype=self.weight_dtype)
            setup_module_for_svd_diff(self.text_encoder_2, scale=1.0, deltas_path=None, qkv_only=self.config.qkv_only)
            self.text_encoder_2.train()
        ...
        self.params_to_optimize.append(
            {'name': 'unet+text_encoders', "params": delta_params_to_optimize}
        )
        self.params_to_optimize.append(
            {'name': 'unet+text_encoders 1d', "params": delta_params_to_optimize_1d, "lr": self.config.learning_rate_1d}
        )
        ...
        print(
            '# params UNet: {0}, # params TE: {1}, # params TE: {2}, # params: {3}'.format(
                unet_numel, te_numel, te_2_numel, unet_numel + te_numel + te_2_numel
            )
        )
    ...
    def save_model(self, epoch):
        ...
        if self.config.finetune_text_encoder:
            ...
            torch.save(
                {name: param for name, param in self.text_encoder_2.state_dict().items() if '.delta' in name},
                os.path.join(save_path, 'text_encoder_2.bin')
            )
