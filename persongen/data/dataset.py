...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_img = self.process_img(image_file)
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example["instance_prompt_ids"] = instance_input.input_ids
        example["instance_attention_mask"] = instance_input.attention_mask
        ...
        if self.class_data_root:
            ...
            example["class_prompt_ids"] = class_input.input_ids
            example["class_attention_mask"] = class_input.attention_mask
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    input_attn_masks = [example["instance_attention_mask"] for example in examples]
    ...
    if with_prior_preservation:
        ...
        input_attn_masks += [example["class_attention_mask"] for example in examples]

    pixel_values = torch.stack(pixel_values)
    pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()

    input_ids = torch.cat(input_ids, dim=0)
    input_attn_masks = torch.cat(input_attn_masks, dim=0)

    batch = {
        "input_ids": input_ids,
        "pixel_values": pixel_values,
        "input_attn_masks": input_attn_masks
    }
    return batch
