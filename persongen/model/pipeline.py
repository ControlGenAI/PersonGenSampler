...
from transformers import (
    CLIPFeatureExtractor, CLIPTextModel, CLIPTextModelWithProjection, CLIPTokenizer, CLIPVisionModelWithProjection
)
...
class StableDiffusionPipelineMultiStage(StableDiffusionXLPipeline):
    ...
    def _encode_prompt(
        ...
        pooled_prompt_embeds = pooled_prompt_embeds.view(num_images_per_prompt, -1)

        ref_prompt_embeds = ref_prompt_embeds.repeat(1, num_images_per_prompt, 1)
        ref_pooled_prompt_embeds = ref_pooled_prompt_embeds.repeat(1, num_images_per_prompt)
        # (num_images_per_prompt, seq_len, dim)
        ref_prompt_embeds = ref_prompt_embeds.view(num_images_per_prompt, seq_len, -1)
        # (num_images_per_prompt, dim)
        ref_pooled_prompt_embeds = ref_pooled_prompt_embeds.view(num_images_per_prompt, -1)
        ...
        if do_classifier_free_guidance:
            ...
            # (num_images_per_prompt, dim)
            negative_pooled_prompt_embeds = negative_pooled_prompt_embeds.view(num_images_per_prompt, -1)
    ...
    def prepare_inputs(
        ...
        add_time_ids = add_time_ids.to(self.unet.device).repeat(self.batch_size * num_images_per_prompt, 1)
    ...
    def __call__(
        ...
        latents, add_time_ids = self.prepare_inputs(
...
class StableDiffusionPipelinePhotoswap(StableDiffusionPipelineMultiStage):
    def __init__(
        ...
        super().__init__(
            ...
            scheduler=scheduler, image_encoder=image_encoder, feature_extractor=feature_extractor
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(self.timesteps):
                if t <= st:
                    ...
                    amap = get_attention_map(self.unet, save_output, word_id, id_in_subbatch=2, final_size=(128, 128))
                    save_output.clear()
