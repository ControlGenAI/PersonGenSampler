...
# noinspection PyPep8Naming
class SVDT5LayerNorm(T5LayerNorm):
    # noinspection PyMissingConstructor
    def __init__(self, layer_norm: T5LayerNorm, initialized: bool = True, scale: float = 1.0):
        # noinspection PyTypeChecker
        T5LayerNorm.__init__(
            self,
            hidden_size=layer_norm.weight.shape[0],
            eps=layer_norm.variance_epsilon
        )
    ...
    def forward(self, x: torch.Tensor, *args: list, **kwargs: dict):
        ...
        if weight_updated.dtype in [torch.float16, torch.bfloat16]:
            hidden_states = hidden_states.to(weight_updated.dtype)

        return weight_updated * hidden_states
    ...
    for name, submodule in tqdm.tqdm(module.named_modules(), desc='Process modules'):
        ...
        for attribute_name in tqdm.tqdm(attributes_names, desc='Process attributes:', total=len(attributes_names), leave=False, disable=True):
            if qkv_only and not (
                ...
                ('SelfAttention' in name and 'q' in attribute_name) or
                ('SelfAttention' in name and 'k' in attribute_name) or
                ('SelfAttention' in name and 'v' in attribute_name)
            ):
                continue
            ...
            if isinstance(attribute, torch.nn.Conv1d):
                ...
                if not attribute.elementwise_affine:
                    continue
                setattr(submodule, attribute_name, SVDLayerNorm(attribute, initialized=True, scale=scale))
            elif isinstance(attribute, T5LayerNorm):
                setattr(submodule, attribute_name, SVDT5LayerNorm(attribute, initialized=True, scale=scale))
            ...
            if isinstance(
                    attribute,
                    (torch.nn.Conv1d, torch.nn.Conv2d, torch.nn.LayerNorm, T5LayerNorm, torch.nn.GroupNorm, torch.nn.Linear, torch.nn.Embedding)
            ):
