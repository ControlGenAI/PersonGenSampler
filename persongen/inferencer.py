...
from diffusers import (
    PixArtAlphaPipeline, AutoencoderKL, DPMSolverMultistepScheduler, PixArtTransformer2DModel
)
...
class BaseInferencer:
    ...
    def setup_pipeline(self):
        self.pipe = PixArtAlphaPipeline.from_pretrained(**self._base_init_pipe_kwargs)
    ...
    def generate_with_prompt(self, prompt, num_images_per_prompt, batch_size):
        ...
        shape = (
            num_images_per_prompt, self.pipe.transformer.config.in_channels,
            self.pipe.transformer.config.sample_size, self.pipe.transformer.config.sample_size
        )
        latents = randn_tensor(shape, generator=generator, dtype=self.pipe.transformer.dtype, device=self.pipe.transformer.device)
...
class SVDDiffBaseInferencer(BaseInferencer):
    def setup_model(self):
        ...
        if self.config['finetune_transformer']:
            ...
            setup_module_for_svd_diff(
                self.transformer, scale=self.args.svd_scale, qkv_only=self.config['qkv_only'],
                deltas_path=os.path.join(self.checkpoint_path, 'transformer.bin'), fuse=True
            )
...
class MultiStageInferencer(BaseInferencer):
    def setup_model(self):
        transformer_path = os.path.join(self.checkpoint_path, 'transformer.bin')
        te_path = os.path.join(self.checkpoint_path, 'text_encoder.bin')

        if os.path.exists(transformer_path):
            self.transformer.load_state_dict(torch.load(transformer_path, weights_only=True))
